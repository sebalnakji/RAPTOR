...
def docx_to_pdf_main(catalog_path, origin_file_name):
    ...
    pdf_file_path = docx_to_pdf(bucket_name, dir_path, origin_file_name)
...
def docx_to_pdf(bucket_name, dir_path, origin_file_name):
    temp_dir = tempfile.mkdtemp()
    base_name = os.path.splitext(os.path.basename(f"{dir_path}/{origin_file_name}"))[0]
    temp_file_path = os.path.join(temp_dir, f"{base_name}.docx")
